    ...
    class DeletePresentationSession
    {
        static void execute(String[] args)
        {
            try
            {
                ...
                {
                    CreateDocumentResponse sheetResponse = (CreateDocumentResponse)response.Object;

                    Console.WriteLine("presentation session id to be deleted - {0}", sheetResponse.SessionId);

                    APIResponse<ShowResponseHandler> response1 = sdkOperations.DeletePresentationSession(sheetResponse.SessionId);

                    responseStatusCode = response1.StatusCode;

                    if (responseStatusCode >= 200 && responseStatusCode <= 299)
                    {
                        SessionDeleteSuccessResponse deleteResponse = (SessionDeleteSuccessResponse)response1.Object;

                        Console.WriteLine("presentation session delete status - {0}", deleteResponse.SessionDelete);
                    }
                }
                else
                {
            ...
            {
                Console.WriteLine("Exception in deleting presentation session - ", e);
            }
        }
