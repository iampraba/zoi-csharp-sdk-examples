        ...
        static void execute(String[] args)
        {
            try
            {
                ...
                if (responseStatusCode >= 200 && responseStatusCode <= 299)
                {
                    ...
                    Console.WriteLine("Presentation session 1 id - {0}", showResponse.SessionId);
                    Console.WriteLine("Presentation session 1 url - {0}", showResponse.DocumentUrl);
                    ...
                    APIResponse<ShowResponseHandler> response1 = sdkOperations.CreatePresentation(parameters);

                    responseStatusCode = response1.StatusCode;

                    if (responseStatusCode >= 200 && responseStatusCode <= 299)
                    {
                        showResponse = (CreateDocumentResponse)response.Object;

                        Console.WriteLine("Presentation id - {0}", showResponse.DocumentId);
                        Console.WriteLine("Presentation session 2 id - {0}", showResponse.SessionId);
                        Console.WriteLine("Presentation session 2 url - {0}", showResponse.DocumentUrl);
                    }
                }
                else
                {
            ...
            {
                Console.WriteLine("Exception in creating presentation session url - ", e);
            }
        }
