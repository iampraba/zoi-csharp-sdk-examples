        ...
        static void execute(String[] args)
        {
            try
            {
                ...
                if (responseStatusCode >= 200 && responseStatusCode <= 299)
                {
                    ...
                    Console.WriteLine("Sheet session 1 id - {0}", sheetResponse.SessionId);
                    Console.WriteLine("Sheet session 1 url - {0}", sheetResponse.DocumentUrl);

                    SheetUserSettings usersSettings = new SheetUserSettings();

                    userSettings.DisplayName = "Praba";

                    parameters.UserInfo = userSettings;

                    APIResponse<SheetResponseHandler> response1 = sdkOperations.CreateSheet(parameters);

                    responseStatusCode = response1.StatusCode;

                    if (responseStatusCode >= 200 && responseStatusCode <= 299)
                    {
                        CreateSheetResponse session2Response = (CreateSheetResponse)response.Object;

                        Console.WriteLine("Sheet id - {0}", session2Response.DocumentId);
                        Console.WriteLine("Sheet session 2 id - {0}", session2Response.SessionId);
                        Console.WriteLine("Sheet session 2 url - {0}", session2Response.DocumentUrl);
                    }
                }
                else
                {
            ...
            {
                Console.WriteLine("Exception in editing sheet session url - ", e);
            }
        }
