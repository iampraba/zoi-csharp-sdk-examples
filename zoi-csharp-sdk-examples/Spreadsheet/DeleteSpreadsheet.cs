    ...
    class DeleteSpreadsheet
    {
        static void execute(String[] args)
        {
            try
            {
                ...
                if (responseStatusCode >= 200 && responseStatusCode <= 299)
                {
                    ...
                    Console.WriteLine("Sheet id to be deleted - {0}", sheetResponse.DocumentId);

                    APIResponse<SheetResponseHandler> response1 = sdkOperations.DeleteSheet(sheetResponse.DocumentId);

                    responseStatusCode = response1.StatusCode;

                    if (responseStatusCode >= 200 && responseStatusCode <= 299)
                    {
                        Console.WriteLine("response1 - {0}", response1.Object);
                        FileDeleteSuccessResponse deleteResponse = (FileDeleteSuccessResponse)response1.Object;

                        Console.WriteLine("Sheet delete status - {0}", deleteResponse.DocDelete);
                    }
                }
                else
                {
            ...
            {
                Console.WriteLine("Exception in deleting sheet session url - ", e);
            }
        }
        ...
        static Boolean initializeSdk()
        {
            ...
            try
            {
                ...
                Com.Zoho.Dc.DataCenter.Environment environment = new DataCenter.Environment("", "https://sheet.zoho.com", "", "");
