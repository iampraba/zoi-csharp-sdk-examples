    ...
    class PreviewSpreadsheet
    {
        static void execute(String[] args)
        {
            try
            {
                ...
                SheetPreviewParameters parameters = new SheetPreviewParameters();
                ...
                APIResponse<SheetResponseHandler> response = sdkOperations.CreateSheetPreview(parameters);
                int responseStatusCode = response.StatusCode;

                if (responseStatusCode >= 200 && responseStatusCode <= 299)
                {
                    SheetPreviewResponse previewResponse = (SheetPreviewResponse)response.Object;

                    Console.WriteLine("Sheet Preview URL - {0}", previewResponse.PreviewUrl);
                }
                else
                {
            ...
            {
                Console.WriteLine("Exception in creating sheet preview url - ", e);
            }
        }
